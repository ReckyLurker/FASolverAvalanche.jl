...
@inline function central_interpolate(face_center1::Meshes.Point3, face_center2::Meshes.Point3, edge_center::Meshes.Point3, vars::Vector)::eltype(vars)
    l = length(vars)
    if l != 2
...
function central_interpolate(face_center1::Meshes.Point3, face_center2::Meshes.Point3, edge_center::Meshes.Point3, local_coords1::Vector{Meshes.Vec3}, local_coords2::Vector{Meshes.Vec3}, vars::Vector)::eltype(vars)
    l = length(vars)
    if l != 2 
        throw("Expected 2, got $l")
    end
    ...
    return eltype(vars)(LinAlg.transpose(Te) * (frac * Tp * vars[1] + (1-frac) * Tn * vars[2]))
end
    ...
    Threads.@threads for i in eachindex(Cells)
        for j in eachindex(Cells[i].neighbours)
            ...
            function pressureContribution(x)
                ...
                hvalues = x[1:l]
                pvalues = x[l+1:end]
                h_edge = interpolator(Cells[i].center_coords, Cells[idx].center_coords, Cells[i].edge_centers[j], hvalues)
                p_edge = interpolator(Cells[i].center_coords, Cells[idx].center_coords, Cells[i].edge_centers[j], pvalues)
...
# Allows user-defined interpolation functions [which are linear, incorrect results may be obtained for discontinuous/non-linear interpolators] 
# Compute Hessians for pressure at edge equations [constraint and momentum eqns]
function computeHessians!(hessians, Cells; interpolator=central_interpolate)
    Threads.@threads for i in eachindex(Cells)
        for j in eachindex(Cells[i].neighbours)
            idx = Cells[i].neighbours[j]
            function pressureContribution(x)
...
function initializePressure!(Cells, alpha, zeta, rho; atol=1e-6, solver=LinSolv.KrylovJL_GMRES())
 ...
 #    For each Cell 
     Threads.@threads for i in eachindex(Cells)
         A[i,i] = rho_inv * Cells[i].area
         B[i] = Cells[i].det * LinAlg.dot(Cells[i].face_normal, g) * Cells[i].h * Cells[i].area 
         for j in eachindex(Cells[i].neighbours)
...
# Currently limited to 2 values for interpolation at edge, fix this in future!
# Need symbolics for discontinuos/non-linear interpolators. Using ForwardDiff for general linear interpolators, with user defined solver. 
function initializePressure!(Cells, alpha, zeta, rho, points; atol=1e-6, solver=LinSolv.KrylovJL_GMRES())
   A = ESparse.ExtendableSparseMatrix(length(Cells), length(Cells))
   B = zeros(length(Cells))
   rho_inv = 1/rho 
   hessians = ESparse.ExtendableSparseMatrix(length(Cells), length(points))
   computeHessians!(hessians, Cells)
#    For each Cell 
    Threads.@threads for i in eachindex(Cells)
        A[i,i] = rho_inv * Cells[i].area
        B[i] = Cells[i].det * LinAlg.dot(Cells[i].face_normal, g) * Cells[i].h * Cells[i].area 
    ...
    for idx in eachindex(Cells)
        Cells[idx].pb = sol.u[idx]
    end
end

# Pressure is computed from constraint equations 
# Initialize the discretized geometry [h and vel] by a bounding polygon(npoints)
function initializeGeometry(coord_bounds, Cells; npoints=4, h0 = 0.0, u0 = Meshes.Vec3(0.0,0.0,0.0), epsilon = 1e-6)
    cells_inside_polygon = cellsInsideBoundingPolygon(Cells, coord_bounds, npoints=npoints, epsilon=epsilon)
    # Set Initial Conditions 
    for idx in cells_inside_polygon 
        Cells[idx].h = h0 
        Cells[idx].vel = u0
    end
end