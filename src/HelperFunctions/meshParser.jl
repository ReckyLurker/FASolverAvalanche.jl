...
function parsePoints(location::String)::Vector{Meshes.Point}
    t_start = Base.time()
    ...
    Threads.@threads for i in eachindex(s_points_cloud)
        ...
        mesh_points[i] = Meshes.Point(parse(Float64, s[1]),parse(Float64, s[2]), parse(Float64, s[3]))
        ProgressBars.update(pbar_points)
    end
...
function parseFaces(location::String)::Tuple{Vector{Vector{Int}}, Int}
    t_start = Base.time()
    ...
    Threads.@threads for i in eachindex(s_faces_arr)
        ...
        mesh_faces[i] = vertex_idxs[2:end]
        ProgressBars.update(pbar_faces)
    end
...
function parseFaceLabels(location::String, mesh_faces::Vector{Vector{Int}}, correction_factor::Int)
    ...
    idx_surface_aligned_faces = [parse(Int, p)+1+correction_factor for p in s_surface_aligned_faces_arr]
